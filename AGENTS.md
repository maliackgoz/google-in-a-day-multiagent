...
| Action | Command |
|--------|---------|
| Run app | `python3 run.py` — HTTP on port **3600** |
| Verify | `python3 verify_system.py` (from repo root) |

`data/` is created at runtime and is gitignored.
...
| Agent | Doc | Primary code / artifacts |
|-------|-----|---------------------------|
...
| **Index & storage engineer** | [`agents/index_storage_agent.md`](agents/index_storage_agent.md) | `storage/file_store.py`, `data/` layout, `readme.md` storage description |
| **Search engineer** | [`agents/search_agent.md`](agents/search_agent.md) | `search/searcher.py`, `verify_system.py` (search-related checks) |
...
- **On-disk JSON, locks, visited/word/job stores**: **Index & storage engineer**; do not change `WordStore` public API without checking crawler/search call sites.
...
- **End-to-end checks, regressions, homework acceptance themes**: **QA / integration engineer**; extend `verify_system.py` in the existing `check(condition, message)` style.
